...
if os.path.isfile('wildfires_dataset.csv') == False:
    ...
    df_subsample = df_train.query('(date > "2011") & (date < "2019")').sample(n=2000)
    ...
    df_train = pandas.read_csv('data/wildfires_check.csv')
    df_subsample = df_train.query('(date > "2011") & (date < "2019")').sample(n=2000)
    ...
    for i, row in tqdm(df_subsample.iterrows(), total=df_subsample.shape[0]):
        ...
        df_features.append(features)
    df_features = pandas.DataFrame(df_features)
    df_features.to_csv("wildfires_test_dataset.csv")
...
torch.save(fireDataset, "fireTestDataset")